...
- `pilot set-admin-password`: Set the admin panel password in `bench.toml`; prompts when `--password` is omitted on a TTY. In non-interactive environments (CI, scripts), `--password` is required — omitting it raises an error to prevent credential exposure in logs. The password must meet the same rules the dashboard enforces.
