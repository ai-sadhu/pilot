...
- `pilot set-admin-password`: set the Admin panel password in `bench.toml`; prompts when `--password` is omitted and stdin is a TTY. Auto-generation (blank prompt input) requires stdout to also be a TTY — if stdout is redirected, supply `--password` to avoid credential exposure in logs. The password must meet the same rules the dashboard enforces.
